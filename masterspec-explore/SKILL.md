...
Две фазы: сначала опись массива, потом извлечение по ней. Порядок не переставляется — без описи нечем нарезать границы, и лид начнёт читать сырьё сам, чего делать не должен.

1. **Awareness.** Как в code-ветке. Переиспользуй существующий research только если совпали `source`, `docs`-пути, mtime < 24 ч и материалы не менялись.

2. **Сайзинг массива.** Посчитай файлы и СЛОВА (`research-orchestration.md § 2.1`) — у документа вес не в файлах, а в словах. Это определит число субагентов фазы B и потолок батча (~40k слов).

3. **Фаза A — опись и разметка.** Один субагент `docs-inventory` (промпт — `references/research-roles.md`, границы = `docs`-пути). Не параллелить: опись должна быть цельной, иначе дубли и границы батчей не сойдутся.

   На выходе — `<research-dir>/.research/docs-inventory.yaml`: каждый файл классифицирован (тип материала, объём, слои-мишени, `machine_ready`, `duplicate_of`, `oversized`), плюс `totals` и предложение `batches[]` с `est_words`.

4. **Проверь предложение описи.** Батч тяжелее потолка — дроби по материалам (не по слоям: слои режутся ролями). Материал с `oversized: true` — раздавай по разделам, не файлом целиком. Молчаливая деградация большого документа в переполненном окне недопустима: лучше честный `gap`.

5. **Определи состав фазы B по описи.** Базово — `requirements-scope`, `components-scope`, `scenarios-scope`, `data-scope` в docs-варианте (`research-roles.md § scope-роли в docs-варианте`). `testcases-scope` — ТОЛЬКО если опись нашла `material_type: test-cases`. `codemap-scope` не применяется: кода здесь нет.

   Группировка по размеру массива — `research-orchestration.md § 3.3`.

6. **Фаза B — запусти субагентов параллельно.** Одним блоком tool-use. Каждой роли в промпт: границы = список материалов из её батча; требование провенанса `source: <path>#<якорь>`; `declared: true` на находках; запрет пересказывать `machine_ready`-материал.

7. **Собери YAML-выводы** в `.research/<role>.yaml`. Валидация — `invocation-contract.md § 6`.

8. **Агрегация.** Как в code-ветке, плюс две docs-коллекции: `machine_artifacts[]` (готовые контракты — путь, формат, перечень операций; тело остаётся в файле) и `test_cases[]` (с `traces_to`). Запиши `_aggregate.yaml` + `.research-notes.md`.

9. **Проверь полноту и стыковку.** Слои без единого источника — в `gaps[]`. Тест-кейсы с `traces_to.confidence: none` — в `gaps[]` поимённо: висячий `tc-` не заводится молча.

10. **Верни сводку** + пути к `docs-inventory.yaml` и `.research-notes.md`. Статистика: сколько материалов разобрано, сколько machine-ready взято как есть, сколько ТК стыковано / не стыковано.
