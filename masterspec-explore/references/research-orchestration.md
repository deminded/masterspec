...
**`testcases-scope` — опциональная роль фазы B2** (после B1, ей нужны её кандидаты). Единый предикат включения: опись пометила `tc-acc` или `tc-int` в `target_artifacts` — хоть у файла, хоть у среза. По `material_type: test-cases` НЕ триггерить: раздел с тест-кейсами часто сидит внутри выгрузки Confluence, и по типу файла он не виден. Состав фазы B определяет опись, а не лид на глаз. Готовые ТК не забираются as-is: каждый обязан стыковаться с элементом спеки (`traces_to` с `confidence`), нестыкованный уходит в открытые вопросы — висячий `tc-` не проверяем на полноту и молча разъедется со спекой при первом изменении.
