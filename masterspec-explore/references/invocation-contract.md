...
- `prompt` — полный текст роли из `research-roles.md` дословно, с подставленными путями.
  - **docs-профиль (`source=docs`).** Роли фазы B1 переиспользуют YAML-контракт кодовой роли, но промпт собирается по `research-roles.md § Фаза B1`: своя шапка («источник — ДОКУМЕНТЫ, не код»), границы = срезы из батча описи (а не пути кода), обязательные поля находки `source: <path>#<якорь>` и `declared: true`, запрет пересказывать machine-ready. Это НЕ вольная правка промпта: профиль задан в каталоге ролей и применяется целиком, без отсебятины. Роли `docs-inventory` и `testcases-scope` — самостоятельные промпты, берутся дословно.
...
> **Находки с `declared: true` (docs) НЕ схлопываются с находками из кода.** Это разные факты: заявленное против фактического. Их сводит сверка в `recover source=both` (отдельный шаг с ключами сопоставления), а не дедупликация — иначе расхождение «в доке написан retry, в коде его нет» тихо исчезнет как «дубль».
...
| `machine_artifacts[]` (только docs) | `path` |
| `test_cases[]` (только docs) | `name` + `source` |
...
2. **Уже есть research рядом с артефактом.** Reuse — ТОЛЬКО по манифесту `<research_root>/_meta.yaml`: должны совпасть `source`, `target`, `roots`/`docs`, `anchor` и отпечаток входа (код — HEAD + `git status --short`; docs — sha256 манифеста `path+size+sha256`). Манифеста нет или хоть одно поле разошлось — собирай заново. `mtime` сам по себе ничего не доказывает: он не отличает другой `source`, другие корни и правку внутри файла.
