...
## Цель
1. Описать все внешние API (REST/gRPC/SSE/WebSocket/GraphQL) как логические контракты (кандидаты на api-*, scope=external).
2. Описать внутренние API между компонентами (кандидаты на api-*, scope=internal) — если ясны из интерфейсов/портов.
