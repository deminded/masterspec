...
3-imported. Если компаньон несёт `contract_origin: imported` — **пропустить, форму не трогать**: контрактом владеет внешняя сторона, migrate его не переразлагает (правка чужого контракта у себя = молчаливый форк). Запись в report: «imported — вне владения фабрики». Изменение такого контракта — `evolve entry=ext` + ре-импорт.
