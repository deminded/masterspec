...
- **Не трогает исходный код и артефакты фабрики.** Единственные записи — `.research/*.yaml` и `.research-notes.md`. Фиксация артефактов — через `derive`/`evolve` (с вычиткой и гейтом, затем apply).
...
- Передавай генератору (`gen`) ИЗОЛИРОВАННЫЙ фокус-набор под его артефакт, а не всю фабрику. Если генератору не хватило — он шлёт дозапрос «не хватает X», ты добираешь точечно, не более 2 раундов, потом эскалация человеку (открытый вопрос).
