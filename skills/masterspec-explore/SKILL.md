...
description: >
  ...
  «картируй зону change», «собери research-notes для propose». Дополнительно работает как
  prompt-library для `masterspec-propose` и kernel `masterspec` (режимы codemap/reverse) —
...
argument-hint: "[target: factory-spec|factory-change] [factory=<slug>] [name=<change>] [anchor=<paths|slugs>]"
license: MIT
compatibility: >
...
2. **Prompt-library** для `masterspec-propose` и kernel `masterspec` (режимы `codemap`/`reverse`). Эти скиллы читают `references/research-roles.md` и `references/invocation-contract.md` и оркестрируют субагентов сами.
...
- Иначе один раз `Glob("**/masterspec-skills/skills/masterspec-explore/references/<file>")` и бери первый результат.
...
1. **Awareness.** Проверь `masterspec/` и активные changes. Подробности — [`references/masterspec-awareness.md`](references/masterspec-awareness.md). Если свежий `.research-notes.md` уже есть (mtime < 24 ч) и `target` совпадает — переиспользуй, субагентов не запускай.
...
10. **Верни вызывающему короткую сводку + путь к `.research-notes.md`**, не сырой YAML. Вызов от пользователя → покажи ту же сводку и предложи следующий скилл (kernel `masterspec` в режиме `design`/`reverse` или `masterspec-propose`). См. `references/masterspec-awareness.md § 5`.
...
- **Не трогает исходный код и артефакты фабрики.** Единственные записи — `.research/*.yaml` и `.research-notes.md`. Фиксация артефактов — через kernel `masterspec` или `masterspec-propose`.
