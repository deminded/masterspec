...
### Graphify как опциональная карта кода (дизайн 16.07, пилоты mailtg-bridge + site)
Контекст: пакет ssot-tools сберовских коллег (разбор 16.07) поверх публичного graphify (MIT, tree-sitter, 36 языков, локально без LLM). Пилоты: их доменная математика переносима на Python (ядро mailtg-bridge cohesion 0.89; site — 3 функциональных домена 0.91–0.97, режет по функции, не по каталогам). Их скрипт JVM-only искусственно + баги (THRESHOLD 0.30 в коде vs 0.15 в доке; хрупкий emit_prefixes) — берём рецепт математики, не код.

**Принцип: строго опционально.** Нет graphify / нет graph.json → каждый скилл работает по-старому без деградации функции (как у коллег: только domain-extraction требует граф). Граф не коммитится (gitignore), строится локально `graphify update .`.

- [ ] **(а) Symbol-anchored provenance** — независимо от graphify, первой очередью: норма `code:<path>#<symbol>` (указывает на определение; `:line` только для анонимных элементов + фиксация в белых пятнах). Правка конвенции provenance + шаблонов; переживает рефакторинги, в отличие от line-дрейфа.
- [ ] **(б) explore: pre-шаг доменной декомпозиции.** Опция `graph=auto|on|off` (auto: graphify в PATH и массив крупный → строить). Новый детерминированный скрипт `masterspec/scripts/graph-domains.py` (stdlib, по рецепту коллег: семантические рёбра по `relation`, cosine-нормализация против хабов, fan-in карантин shared, union-find, absorb, cohesion без shared в знаменателе; + наш префильтр source_file по кодовым расширениям — graphify экстрактит код из fenced-блоков md; БЕЗ языкового гейта; дефолт порога = задокументированный, негативный тест на расхождение код↔дока — урок коллег). Выход `domains.json` → нарезка scope разведчиков по функциональным доменам вместо эвристики лида. Fallback: текущая нарезка по слоям/материалу.
- [ ] **(в) codemap: граф как подсказка.** При наличии graph.json — cmap-/trace- строятся с готовыми символьными связями (соседи/вызовы из графа), приоритет инструментов: graphify-карта → Serena (наведение/верификация) → LSP → grep. Fallback: текущий приоритет без графа. Рассмотреть graphify-mcp как чистый интерфейс «спроси граф» для скиллов.
- [ ] **(г) masterspec-sync (git-diff → устаревание) — отдельной спекой, вторая очередь.** Вход: диапазон коммитов; git diff → семантическая классификация изменённых файлов → blast-radius (при графе — соседи по рёбрам; fallback — импортёры через Serena/grep) → маппинг на артефакты по provenance/codemap → черновики обновлений (draft) → verify → human-gate. Закрывает O_T5-свежесть механикой вместо эвристики дат. Смежное: Coverage Inventory с блокирующим гейтом (таблица «элемент кода → где документирован | out of scope: причина») как verify-ось полноты codemap.
- [ ] **Мелочи из ssot-tools по ходу:** волна question-resolver для параллельных gen-узлов (связать вопросы, отвеченные соседним артефактом; изобретать ответы запрещено); completion-маркеры «артефакт на диске = успех» + retry-лестница; запрет inline-[TBD] (вся неопределённость — в хвостовой секции).
