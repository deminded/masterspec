...
>
> Каждый скилл = `SKILL.md` + его `references/`: при выполнении операции references обязательны (промпты ролей, контракты вызова субагентов, алгоритмы мержа) — это часть скилла, не опциональное чтение. Оркестраторы (`derive`/`evolve`) используют встроенный механизм субагентов (`Task`); если его нет — работают последовательно (fallback описан в скилле).
>
