...
| `explore` | 👤→🤖 | разведка кодовой базы под мета-модель: параллельные read-only субагенты по 5 ролям → агрегат в `.research/`. Отдельно и как первый шаг `derive`/`evolve` |
| `recover` | 👤 | восстановить описание из того, что уже есть — из документов и/или кода; помечает происхождение (`provenance`) и «Белые пятна» |
...
| `propose` | ⛔ | DEPRECATED — заведение change поглощено `evolve` (шаг 0). Оставлен как заглушка, напрямую не активируется |
...
| `explore` | `[target=factory-spec\|factory-change]` `roots=<пути к коду>` `[factory=<slug>]` `[name=<change>]` `[anchor=<paths\|slugs>]` — `target` выбирает охват: вся фабрика или только зона изменения (`anchor` сужает роли до нужных путей/узлов) |
| `recover` | `source=docs\|code` `[roots=<корневые пути кода>]` `[context=]` — `docs`: ТЗ, презентации, переписка, внешние спеки; `code`: обратное восстановление спеки + codemap (тогда нужен `roots=`) |
