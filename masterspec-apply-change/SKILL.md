...
- **Гейт `contract_origin` (перед любой записью в сайдкар).** Прочитай компаньон целевой пары. Если `contract_origin: imported` — записывать в файл из `sidecar:` ЗАПРЕЩЕНО: контрактом владеет внешняя сторона, и подмена его нашим change = молчаливый форк (локально «починили», в проде разъехались). Допустимы ровно две операции, и обе объявляются в change явно:
  - `reimport` — сайдкар заменяется НОВОЙ ревизией от внешнего владельца (change несёт обновлённый `contract_source: {uri, revision, sha256}`; дайджест обязан сойтись с содержимым файла);
  - `adopt` — акт присвоения: `contract_origin` переводится в `authored` решением `dr-` (без него — стоп).
  Компаньон (семантика: маппинги, SLA, связи) правится обычным diff-блоком всегда — он наш.
