...
1. Построй план транзакции: какие пары (компаньон + сайдкар) будут затронуты, что пишется в каждую.
2. Для КАЖДОЙ затронутой пары прочитай компаньон в дереве и возьми `contract_origin`.
3. Если `imported` и change пишет в файл из `sidecar:` — запись ЗАПРЕЩЕНА, кроме двух явно объявленных в change операций:
   - **`reimport`** — сайдкар заменяется новой ревизией ОТ ВЛАДЕЛЬЦА: change несёт обновлённый `contract_source: {uri, revision, sha256}`, и `sha256` обязан сойтись с содержимым нового файла. Не сошёлся — стоп: это не ре-импорт, а подмена;
   - **`adopt`** — акт присвоения: `set-frontmatter contract_origin: imported → authored` + решение `dr-` в том же change. Без `dr-` — стоп.
   Иначе — стоп с объяснением: контрактом владеет внешняя сторона, правка у себя = молчаливый форк (локально «починили», в проде разъехались).
4. Компаньон (семантика: маппинги, SLA, связи) правится обычным diff-блоком ВСЕГДА — он наш.
5. Только когда весь план прошёл гейт — применяй. Частично применённый change с отклонённой парой недопустим: дерево останется в состоянии, где половина изменения есть, а половина нет.
...
- Вердикт §11 = `confirmed` ИЛИ `override` → change применён. **Но статус ставится ПОСЛЕ сертификации (§13), по её результату** — иначе `Реализовано` появляется раньше, чем известно, соответствует ли результат инвариантам, и `archive-change` заархивирует непроверенное:
  - сертификация дала полный штамп (все применимые scope зелёные) → `> **Статус**: Реализовано`;
  - штамп scope-квалифицированный или не поставлен (остались BLOCKER) → `> **Статус**: Применено, не сертифицировано` + перечень остаточных блокеров в change.md. Это НЕ откат: правки в дереве, работа не потеряна. Это честное имя состояния, в котором изменение внесено, а фабрика инвариантам ещё не удовлетворяет.
